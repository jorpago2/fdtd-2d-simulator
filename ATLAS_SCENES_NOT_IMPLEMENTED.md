...
- Quantitative negative-index and superlens validation remains pending. The current 107/108 presets now use passive electric+magnetic ADE material dynamics, but validation should add measured refraction angle or phase-front slope, image transfer/resolution metrics, power-balance residuals, grid/timestep convergence checks, and comparison against a reference solution.
- Quantitative chiral/bianisotropic validation remains pending. The current 78/79 presets now evolve six 2D field components, but a publication-grade validation should calibrate the time-domain `kappa_n` convention against a known reciprocal chiral or omega-medium reference, check passivity/energy balance, and compare polarization conversion against an analytic or independent numerical solution.
