...
- 11 Evanescent isolated wave: implemented as a monochromatic line source with explicit `k_parallel/k0 > 1`, imposed tangential spatial phase, and exponential normal decay.
- 24 In-plane electric dipole: implemented as a localized `Jx/Jy` current source in the `Hz` solver.
- 32 Progressive-phase dipole array: implemented with per-source temporal phase `phaseDeg`; the preset phase step is now derived from a target steering angle rather than a hard-coded phase increment.
...
- 111 Kerr nonlinear slab: implemented as an instantaneous saturated Kerr update on tagged material cells, `epsilon = epsilon_base + chi3 |F|^2`, using the simulator's physical field scale after internal renormalization.
- 112 SHG and 113 THG slabs: implemented qualitatively as nonlinear polarization-current sources, `J_NL = d(chi2 E^2 + chi3 E^3)/dt`, on tagged material cells, with the nonlinear polarization evaluated from the physical field scale.
...
- 116 VO2 switching slab and 117 PCM memory cell: implemented qualitatively with a persistent phase-state variable `s` per tagged cell, field-scale-corrected intensity threshold hysteresis, finite on/off switching times, and default bidirectional amplitude sweeps.
...
- 131 PT guides: implemented qualitatively as balanced gain/loss coupled guides with field-scale-corrected intensity-saturated gain to avoid immediate numerical blow-up. They now default to a `gamma` sweep of the balanced gain/loss magnitude and report a reduced 2x2 coupled-mode estimate of the PT phase.
...
- 121 Temporal interface, 122 temporal slab, 123 temporal epsilon modulation, 124 temporal crystal, 125 temporally modulated guide, 126 traveling modulation, 127 temporal-isolator analogue, 128 modulated ring, 129 Floquet resonators, and 130 synthetic frequency dimension are implemented as examples using tagged sinusoidal epsilon modulation. The uniform temporal cases now cancel residual spatial modulation phase cell-by-cell, while resonator-chain examples use local phase offsets to represent staggered Floquet coupling phases. The FDTD diagnostics accumulate multi-frequency DFT phasors at the left/right line monitors and estimate a reduced two-port Floquet matrix `S_{m,0}` at `f0 + m Omega` for `m = -2..+2`, including transmitted `T_m`, reflected `R_m`, relative phase, sideband powers, total outgoing power, a normalized measured-order power-balance residual `Pout/Pinc - 1`, dominant sideband order, and modulation-phase coherence `Cphi`. The traveling-modulation and isolator-style presets also default to a forward/reverse direction sweep that reports approximate isolation; calibrated multi-incidence temporal scattering matrices remain future work.

## Implemented in the first coupled-workflow pass

- 133 Non-Hermitian skin-effect analogue, 134 BIC + Kerr, 135 BIC + ENZ, 136 Janus + topological guide, 137 Huygens + cavity, 138 topology + temporal modulation, 139 nonreciprocity + Valley Hall, and 140 space-time crystal are implemented as composite presets that combine existing scalar FDTD ingredients. The coupled-workflow diagnostics now report reduced proxies for field centroid, edge/skin accumulation, gain-vs-loss localization, guide/cavity/source overlap, active-material overlap, and modulation-phase coherence. BIC + Kerr and BIC + ENZ also expose the reduced Bloch-k leakage/Q/band proxy; 138-140 expose the reduced Floquet sideband `S_m` readout. These are demonstration scenes, not full modal/eigenvalue or calibrated nonreciprocal-device validations.
...
- 78 Chiral medium and 79 bianisotropic medium now also expose first-pass quantitative six-field observables: material cross-polarized energy fraction, output-line cross-polarized conversion, generated cross fraction, primary/cross output correlation, `kappa_n`-signed convention proxy, `kappa_n` statistics, and the local positive-definite passivity margin `1 - kappa_n^2`. These metrics are shown in the analysis panel, can drive frequency/amplitude sweeps, and are exported to CSV.
...
- 109 Hyperlens now uses a passive radial/tangential Drude ADE tensor rather than a constant positive-anisotropy placeholder. It also exposes comparative FDTD observables for radial field transfer, angular-detail transfer, and a normalized angular-harmonic MTF proxy. It is still a qualitative scalar-`Hz` 2D analogue; quantitative hyperlens validation still needs convergence checks, radial/tangential parameter calibration, and comparison against a reference solution.
- 121-130 temporal/spacetime examples use the current sinusoidal epsilon modulation on tagged material cells. The reduced `S_{m,0}` readout now comes from DFT phasors accumulated in the FDTD line monitors, separates transmitted/reflected orders, and reports a normalized measured-order power-balance residual, but it is still a one-incident-channel, two-monitor estimate rather than a fully calibrated multi-port Floquet scattering matrix. The 127 temporal-isolator scene reports an approximate forward/reverse isolation ratio, but this remains a traveling-modulation analogue rather than a proof of ideal one-way isolation.
