(function initFdtdNextCanvasViewport(global) {
  "use strict";

  const root = global.FdtdNext || (global.FdtdNext = {});
  const canvas = root.canvas || (root.canvas = {});

  function defaultClamp(value, min, max) {
    return Math.min(max, Math.max(min, value));
  }

  function defaultClampInt(value, min, max) {
    return Math.round(defaultClamp(Number(value) || 0, min, max));
  }
  ...
  function createViewportMethods(options = {}) {
    const clamp = options.clampNumber || defaultClamp;
    const clampInt = options.clampInt || defaultClampInt;
  ...
  function installViewportMethods(SimConstructor, options = {}) {
    if (typeof SimConstructor !== "function" || !SimConstructor.prototype) {
      throw new Error("installViewportMethods expects a simulation constructor.");
    }
    Object.assign(SimConstructor.prototype, createViewportMethods(options));
    return SimConstructor;
  }

  canvas.viewport = Object.freeze({
    canvasPixelSize,
    createViewportMethods,
    installViewportMethods,
    viewportForGridView,
  });
})(window);
